...
class Super_search extends Module_builder_super_search
{
	...
	function _cached($hash = '', $type = 'q')
	{
		...
		if ($query->num_rows() > 0)
		{
			$cache	= (
				$query->row('ids') == ''
			) ? array(): $this->_uncerealize($query->row('ids'));
					...
					$field_content = ee()->typography->parse_type(
						...
						array(
							...
							'html_format'		=> (
								...
								$channels[$row['channel_id']][$this->sc->channel . '_html_formatting'] :
								'all',

							'auto_links'		=> (
								isset($channels[$row['channel_id']])
							) ?
								$channels[$row['channel_id']][$this->sc->channel . '_auto_link_urls'] :
								'n',

							'allow_img_url'		=> (
								isset($channels[$row['channel_id']])
							) ?
								$channels[$row['channel_id']][$this->sc->channel . '_allow_img_urls'] :
								'y'
						)
					);
	...
	function _parse_uri($str = '')
	{
		...
		$this->grid_fields	= ee()->super_search_lib->grid_fields;
				...
				foreach ($customfields as $field => $val)
				{
					...
					foreach($field_loop as $val)	// I am reusing this $val var on purpose for convenience
					{
						...
						if (! empty($search['field'][$field]['or']))
						{
							...
							elseif (($temp = ee()->super_search_lib->prep_sql('or', $arr)) !== FALSE)
							{
								$fieldor	= array_merge($fieldor, array($temp));
							}
						}
						...
						if (! empty($search['field'][$field]['not']))
						{
							...
							elseif (($temp = ee()->super_search_lib->prep_sql('not', $arr)) !== FALSE)
							{
								$not	= array_merge($not, array($temp));
							}
						}
